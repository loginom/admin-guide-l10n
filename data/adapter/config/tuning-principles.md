...
При взаимодействии с внешними сервисами WSP использует только POST-запросы, GET-запросы не используются.
...
Кроме отправки и получения сообщений web-сервис WSP формирует WSDL-описание сервиса. Получить WSDL-описание можно в браузере на компьютере с развернутым WSP по URL: `http://localhost/LoginomAdapter/Service.svc?wsdl`. При обращении к веб-сервису из сети можно использовать URL: `http://<ip-адрес>/LoginomAdapter/Service.svc?wsdl` , где `<ip-адрес>` - это ip-адрес компьютера с развернутым Loginom Adapter.
...
Параметры *Имя веб-сервиса*, *Имя метода*, *Имя метода в WebServiceProxy* участвуют в формировании имен элементов WSDL-описания сервиса. Например, значения параметров *Имя веб-сервиса* формируют уникальные имена элементов `<wsdl:portType>`, описывающих сервисы, предоставляемые веб-сервисом адаптера, а значения параметров *Имя метода в WebServiceProxy* формируют уникальные имена элементов `<wsdl:operation>`, описывающих методы этих сервисов.

Так же, в формировании WSDL участвует обязательный компонент настроек – XSD-схема описания сервиса, путь к которой указывается в параметре *Схема, на которую будет ссылаться описание сервиса*. Данная схема должна определять XML-структуру ответов и запросов всех методов каждого сервиса, описанного в файле конфигурации web.config, и импортируется в WSDL в разделе `<wsdl:types>`. Определения всех сообщений запросов и ответов описываются отдельными элементами импортируемой схемы, и представлены узлами WSDL `<wsdl:message>`.
...
Таким образом, XSD-схема описания сервиса должна содержать эти элементы и задавать их структуру(tipe), а из настроек WSP должно быть понятно, какое именно сообщение описывается определенным элементом импортируемой схемы. Для этого для запроса и ответа каждого метода необходимо задать «имя» и «пространство имен» соответствующего элемента схемы. Эти атрибуты однозначно идентифицируют элементы в XSD-схеме, и задаются следующими параметрами:
...
Значения параметров *Описание веб-сервиса* задает содержание узлов WSDL `<wsdl:documentation>`, и используется для произвольного описания сервисов.